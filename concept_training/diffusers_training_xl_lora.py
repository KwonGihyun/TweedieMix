...
sys.path.append('./')
from concept_training.diffusers_model_pipeline_xl_lora import CustomDiffusionAttnProcessor, set_use_memory_efficient_attention_xformers
from concept_training.diffusers_data_pipeline_xl import CustomDiffusionDataset, PromptDataset, collate_fn
from concept_training.diffusers_model_pipeline_xl_lora import CustomDiffusionPipeline
from diffusers.loaders import AttnProcsLayers
from concept_training.diffusers_model_pipeline_xl_lora import LoRAAttnProcessor_base
...
def save_checkpoint(save_path, freeze_model="lora", save_text_encoder=False, all=False, save_weight=True,unet=None,text_encoder=None,text_encoder_2=None,modifier_token=None,modifier_token_id=None,modifier_token_id_2=None):
    if all:
...
def main(args):
    ...
    unet = create_lora_diffusion(unet)
    ...
    if args.modifier_token is not None:
        ...
        if args.freeze_model == 'crossattn':
            ...
            print('yes')
            # print(lora_layers)
            params_to_optimize = itertools.chain( text_encoder_one.get_input_embeddings().parameters(),text_encoder_two.get_input_embeddings().parameters(),lora_layers.parameters())
...
#                 prompt_embeds = prompt_embeds.hidden_states[-2]
                ...
                if accelerator.sync_gradients:
                    params_to_clip = (
                        itertools.chain(lora_layers.parameters(), text_encoder_one.parameters(), text_encoder_two.parameters())
                        if (args.train_text_encoder or args.modifier_token is not None)
